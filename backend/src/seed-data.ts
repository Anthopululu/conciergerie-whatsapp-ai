...
async function seed() {
  ...
  dbQueries.addMessage(conv1.id, 'Bonjour, à quelle heure ouvre la conciergerie demain ?', 'inbound', 'whatsapp:+33612345678');
  ...
  dbQueries.addMessage(conv2.id, 'Je voudrais réserver la salle commune pour samedi prochain', 'inbound', 'whatsapp:+33623456789');
  dbQueries.addMessage(
    conv2.id,
  ...
  dbQueries.addMessage(conv2.id, 'Oui merci, c\'est pour 20 personnes de 14h à 18h', 'inbound', 'whatsapp:+33623456789');
  ...
  dbQueries.addMessage(conv4.id, 'Mes invités arrivent ce soir, comment peuvent-ils accéder au parking ?', 'inbound', 'whatsapp:+33645678901');
  dbQueries.addMessage(
    conv4.id,
  ...
  dbQueries.addMessage(conv4.id, 'Parfait, ils arrivent vers 19h donc ça ira. Merci !', 'inbound', 'whatsapp:+33645678901');
